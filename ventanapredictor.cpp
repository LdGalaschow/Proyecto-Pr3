...
void ventanapredictor::on_textEditor_textChanged()
{
   ...
   QString searchPalabra;
   QString Text = TextEditor->toPlainText();


          QStringList palabras = Text.split(" ",QString::KeepEmptyParts);

   for (int i = 0; i < palabras.size(); ++i)
        searchPalabra = palabras.at(i).toLocal8Bit().constData();

         DynList<string> auxlist = nodo->search_words(searchPalabra.toStdString());
